// Import script for July events from the user-provided file
const fs = require('fs');
...
// Test with the specially provided July CSV with emoji pairs
async function testJulySpecialFile() {
  try {
...
testJulySpecialFile()
  .then(() => {
    console.log('Special July emoji test completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('Special July emoji test failed:', error);
    process.exit(1);
  });