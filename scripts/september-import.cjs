// Import script for September events
const fs = require('fs');
...
// First, let's remove any existing September events
async function clearSeptemberEvents() {
  try {
    console.log('Checking for existing September events...');
    const september2025 = new Date('2025-09-01T12:00:00Z');
    const october2025 = new Date('2025-10-01T00:00:00Z');
    
    const result = await pool.query(
      'DELETE FROM events WHERE date >= $1 AND date < $2 RETURNING id',
      [september2025.toISOString(), october2025.toISOString()]
    );
    
    console.log(`Removed ${result.rowCount} existing September events`);
  } catch (error) {
    console.error('Error clearing September events:', error.message);
  }
}

// Main function to import events
async function importSeptemberEvents() {
  try {
    await clearSeptemberEvents();
    
    const filePath = 'attached_assets/september_events_bulletin.csv';
